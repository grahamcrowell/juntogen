...
- **Per-spawn `model` directive (Moderate + Complex branches)**: BOTH the Moderate (3-phase) branch and the Complex branch MUST instruct the skill to set the Task tool `model` parameter on every spawn per `${CLAUDE_PLUGIN_ROOT}/reference/execution-protocol.md` § Agent Spawning + Model Selection. Emit a terse blockquote at each branch stating that (a) every spawn sets `model` to `platform.model_policy.default_model` - one model for all personas, so a spawn that omits it is a defect rather than a shortcut; and (b) because `platform.effort_binding` is `session` on this target, the manager sets session effort ONCE at triage from `platform.engagement_effort` (Moderate -> the implementation tier's effort, Complex -> the reasoning tier's effort) rather than per spawn. The function-first rules still govern which spawns warrant the session's attention budget (adversarial reviewer slot and Complex-tier lead at {tier-reasoning}; Moderate lead and domain-trigger specialists at {tier-implementation}, escalating to {tier-reasoning} on decisive risk or unresolved TENSION; Phase-1 analysts at {tier-implementation}, dropping to {tier-routine} for bounded lenses), but the blockquote MUST NOT present them as a per-spawn parameter - there is no effort argument on the spawn call, and naming one in a prompt documents intent, not enforcement. Point readers to `${CLAUDE_PLUGIN_ROOT}/reference/execution-protocol.md` § Agent Spawning + Model Selection for the per-role default fallback table; do NOT duplicate the full table in the skill body.

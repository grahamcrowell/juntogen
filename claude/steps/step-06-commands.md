...
- **Spawn-configuration directive (Moderate + Complex branches)**: BOTH the Moderate (3-phase) branch and the Complex branch MUST instruct the skill how spawn configuration resolves, per `${CLAUDE_PLUGIN_ROOT}/reference/execution-protocol.md` § Agent Spawning + Model Selection. Emit a terse blockquote at each branch stating that (a) each role declares its own `model`, `effort` and turn ceiling in its agent definition, so a spawn that OMITS `model` inherits the role's class and is correct - `model` is passed only to *override*, never as boilerplate; (b) the overrides that earn their cost are the adversarial reviewer slot (always - it is the load-bearing critique surface whatever role fills it) and the lead implementer when the work is high-risk or the findings ledger carries an unresolved TENSION, with Phase-1 analysts never promoted because their output is compressed before it reaches the implementer; and (c) because `platform.effort_binding` is `per-agent` on this target, session effort covers the MANAGER's own turns only, is set ONCE at triage from `platform.engagement_effort`, and is **never raised mid-run** - changing effort between requests invalidates the prompt cache, so a mid-engagement raise re-writes the accumulated prefix at cache-write price. Point readers to `${CLAUDE_PLUGIN_ROOT}/reference/execution-protocol.md` § Agent Spawning + Model Selection for the per-role class-and-tier table; do NOT duplicate the full table in the skill body. **Mandatory pair runs inline at Moderate**: the generated Moderate branch MUST instruct the skill that the mandatory Product + Distinguished pair is applied INLINE as PERSPECTIVE blocks (the same format Simple tier uses), NOT spawned, and that Phase 1 delegates only stakeholders who would genuinely read different files than the manager already has — a Security read on an auth change, a Data Architect read on a destructive migration — capped at THREE delegated analysts. Phases 2 and 3 stay delegated in every case. Emit the reason, because it is what stops the cap being read as a corner cut: the delegation boundary exists to make peer review possible (Axiom 1), which is load-bearing for an adversarial reviewer that must break work it did not do, and decorative for two mandatory lenses that re-read the manager's own files and hand back a paragraph each. If more than three stakeholders look necessary, that is the escalation guard firing — re-triage rather than widening the fan-out.
...
- **Filing rule at the artifacts step (applies to BOTH cycle Step 10 and run-task Phase 5)**: the artifacts step MUST NOT say "store documents under the artifacts root" and stop there - that instruction is what makes a skill pile unrelated documents into one directory and leave a human to re-file them. Generators MUST emit, at cycle "Step 10" (retitled from "Artifacts" to "File the artifacts") and at the run-task Phase-5 Artifacts sub-section, an instruction to: (1) classify each durable document by TYPE - decision / fact / requirement / design / plan / review / analysis - not by which skill produced it or what subject it covers; (2) resolve that type's key with `oj-helper resolve-path <type>`, passing `--node <relpath>` for the node that owns the subject matter; (3) write to the resolved path on exit 0; (4) on exit 3 fall back to one document under `oj-helper resolve-path artifacts` (fallback `.claude/artifacts/`) - the fallback is what keeps this non-breaking for `flat`-layout projects and MUST be emitted. The emitted prose MUST also name the two rules that decide the ambiguous cases - **a review is not a design** (point-in-time findings to history, new intent to the node, split a document that does both) and **a fact needs provenance** (prefer `ASSERTED-UNVERIFIED` plus a stated verification path over a bare number) - then point at `${CLAUDE_PLUGIN_ROOT}/reference/file-patterns.md` § Filing Rule for the full procedure with an explicit "do not duplicate it here". The reference doc (step-04 § 6) is the single source; the skills carry the summary plus the pointer. Preserve run-task's process-noun hygiene in the emitted text ("this run-task invocation", never "the cycle").
- Constraints section: multi-item loop semantics with a per-iteration single-item scope (one item per loop iteration, NOT one item per cycle invocation); atomic per-item commits with Step-7a clean-tree gate BEFORE the loop advances; stop on blocking review; ask when blocked; explicit budget/safety-gate stop conditions (context past the gate threshold at an item boundary, token budget low, next item triages Complex, irreversible / one-way-door action required, user-only decision reached) — the runner surfaces control to the user when any of these trip

- **Context gate at the item boundary (Step 7b)**: the generated cycle skill MUST emit a context gate immediately after the Step-7a clean-tree gate, and MUST present it as a hard block on loop advancement in exactly the way a dirty working tree is. The emitted prose instructs the skill that if context has passed roughly **150k tokens** at the item boundary, it stops the loop there, runs the per-invocation steps, reports `context-gate` as the stop reason, and tells the user to `/clear` and re-invoke — it does NOT start another item. Register `context-gate` in the stop-reason enumeration in BOTH the Loop & Stop Conditions block and the dev-mode feedback `stop_reason` field, and fold the concrete threshold into stop-gate 1 so "budget is running low" stops being a judgement call deferred until the window is nearly full. The rationale MUST be emitted with the gate, not left implicit: context is re-read in full on every turn, so a loop that keeps taking items pays for its own history repeatedly, and the early half of a long transcript stops informing the work long before it stops being billed. The generated Notify step MUST tell the user to `/clear` unconditionally rather than only "if context is getting large" — by that point it is. Without this directive a regen would silently restore an unbounded loop whose only brake is a suggestion in its final step.

- **Loop is the exception, not the default**: the generated cycle skill's Constraints section MUST state that `/run-task` is the single-item form and is the right reach unless the user is deliberately draining a queue of small related items. This does not alter the cycle-vs-run-task asymmetry directive below — cycle still loops — it sets which command a reader should default to.
...
**Body**: Reproduce the 6-probe runtime protocol from `{OJ_SOURCE}/skills/health-check/SKILL.md`:
...
- Probe 6: state-path resolution and backlog integrity — `oj-helper resolve-path backlog`, `oj-helper backlog-list`, `oj-helper backlog-lint`. Assertions: `resolve-path backlog` exits 0 and prints exactly one absolute path (a non-zero exit means the workspace root was not resolved and every state-touching skill is operating on guesses); any `WARNING:` on `resolve-path` stderr is DEGRADED rather than ignored, since it means an override escapes the `.claude/` state tree while still resolving; `backlog-list` exits 0, with the path count reported and **zero treated as a fact, not a failure**; `backlog-lint` exits 0 or 1, where **exit 1 is DEGRADED, not FAIL** — the plugin is working correctly and reporting a defect in the PROJECT's state, and the emitted prose MUST instruct the skill to keep that distinction visible in its report. Each `backlog-lint` finding is reported verbatim with its class (`DUPLICATE-ANCHOR`, `DUPLICATE-ID`, `ID-COLLISION`).
...
- Phase 5 — Learn: Retrospective, dev-mode feedback path via `oj-helper feedback-path`, artifacts filed by type per the filing-rule directive in the cycle-skill section above (`resolve-path <type>`, falling back to `oj-helper resolve-path artifacts` on exit 3), notify user
...
- Step 2 — Load backlog items via `oj-helper issue-tracker-list --project PROJECT_KEY` (issue-tracker mode) OR, in BACKLOG.md mode, enumerate the file set with `oj-helper backlog-list` and markdown-parse **every** path it prints, aggregating across them. The emitted prose MUST state why: a project with per-node backlogs has no single file holding all the work, so reading one shows a fraction and omits the rest **without saying so**. Fall back to `oj-helper resolve-path backlog` (then `.claude/BACKLOG.md`) only when `backlog-list` prints nothing. Track the owning file per item — Step 3 reports it. When more than one file is in play, also run `oj-helper backlog-lint` and carry its findings into the output; its exit 1 is a report to pass along, not a reason to abort a read-only view.
- Step 3 — Present summary: header (source + total open count), items grouped by priority with ID / Title / Status, omit empty priority groups, highlight next-cycle candidate (single highest-priority unblocked item; tiebreak by oldest creation date). When the listing aggregated several files, the header MUST say how many and name them — a reader who is not told the listing spans five files cannot distinguish it from one that quietly covered one. Report `backlog-lint` findings, or state the structural check was clean, and show the owning file for any ID flagged as colliding, since such an ID no longer identifies a single item.
...
**Purpose**: Scaffold an isolated parallel `/cycle` execution thread — its own directory, its own git worktree on its own branch, and a per-workstream `.claude/CLAUDE.md` enforcing a `[ws: <wsid>]` tagging discipline — while SHARING the workspace's canonical oj state with every other concurrent workstream: `.claude/BACKLOG.md`, session state, artifacts, `oj-paths.env`, and any root-level category files (`decisions.md`, `open-questions.md`, `facts/`) the workspace has. The generated skill description MUST name the resolved set rather than the old literal three, and MUST point at `${CLAUDE_PLUGIN_ROOT}/reference/file-patterns.md` § Filing Rule for how documents produced inside a workstream are filed, noting the `[ws: <wsid>]` tag is carried into every shared-file append. The scaffolding filesystem work is done by the `oj-helper workstream-new` subcommand (step-07); the skill elicits inputs, invokes the helper, and surfaces the helper's next-steps block to the user. Canonical spec: `D56-commands-automation.md` § Workstream Scaffolding Command.

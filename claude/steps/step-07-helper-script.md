...
- **state keys** - `session | backlog | artifacts | state-dir | config | retros | reviews`
...
5. **Per-key defaults** (workspace-relative; oj state lives directly under `.claude/` - there is no `.claude/local/` layout):
   - **State keys, both profiles** - `session` -> `.claude/state/session.md`, `backlog` -> `.claude/BACKLOG.md`, `artifacts` -> `.claude/artifacts`, `state-dir` -> `.claude/state`, `config` -> `.claude`, `retros` -> `.claude/archive/retros`, `reviews` -> `.claude/archive/reviews`. These are profile-INDEPENDENT: relocating them is an override's job, not a taxonomy's, so `hierarchy` must not silently move `session`.
   - **`reviews` is a STATE key, not a category key, and that placement is load-bearing.** A review is a point-in-time finding *about* work, so it ages out as the work changes; filing it at the owning node would make it answer "what is the current design?" when it does not (D48 § `file-patterns.md`, "a review is not a design"). It therefore sits beside `retros` in the history area and resolves under BOTH profiles, rather than joining the category keys that go unset under `flat`. The change-review command MUST resolve this key instead of writing a literal path: a hardcoded review location resolves correctly only in the workspace it was written for, and silently writes outside the state tree in every project that relocated the area via a `reviews=` override.
...
SUBCOMMANDS:
  ...
  resolve-path         Echo the canonical path for a state key
                       state:    session|backlog|artifacts|state-dir|config|retros|reviews

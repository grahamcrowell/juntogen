...
- **Agent Spawning + Model Selection** — the spawning pattern plus the full model-selection content moved from CONDUCTOR Section 7:
  ...
  - **Per-Role Default Tier (adjustable; function rules always win)** table: reasoning tier (Distinguished Engineer, Security Engineer, Site Reliability Engineer, Engineering Consultant); implementation tier (Software Engineer, Solutions Architect, DevOps Engineer, Test Engineer, Data Architect, Data Scientist, ML Engineer, Enterprise Architect); routine tier (Business Analyst, Product Manager, Executive Leadership Coach, Technical Writer - with escalation to implementation when user-facing prose or a decision artifact is the deliverable). Override qualifier: "the function rules above always take precedence when any of them applies (reviewer-slot, Complex-tier lead, Moderate-tier lead, Phase-1 analyst, or domain-trigger specialist); the per-role default below fires only when no function rule matches the spawn."
  - **Fan-Out Budget** — the per-invocation spawn/fan-out budget guidance from D32 §6.
  - **Worked-example anchors**: back-pointer to `${CLAUDE_PLUGIN_ROOT}/reference/worked-examples.md` Example 2, where every spawn carries the same `model` and the tiers differ only in the depth of reasoning each warrants. Plus the reviewer-slot-override anchor: a role whose default is {tier-routine} or {tier-implementation}, spawned as the adversarial reviewer, is a {tier-reasoning} spawn - the reviewer slot takes that tier by function, not by role, so do not read Example 2's reviewer as its role's default.
  - **Effort-binding-site note**: render from `platform.effort_binding`. On this target it is `session`, so state plainly why: profiles are injected into `general-purpose` Task spawns via the `SubagentStart` hook (`oj-helper inject-profile`); the Task tool does NOT read `${CLAUDE_PLUGIN_ROOT}/agents/*.md` as subagent definitions, so their frontmatter is a no-op for spawn config; and the spawn surface exposes no per-invocation effort argument. Effort is therefore set once per engagement from `platform.engagement_effort` via the session effort setting (`platform.session_effort_key`, i.e. `effortLevel` / `/effort`). The note MUST retain the prohibition on fabricating per-expert effort control: naming an effort level in a spawn prompt documents intent, it does not set a parameter. Re-architecting experts as native subagent types remains deferred; other generator targets (see `juntogen/codex/`) do expose a per-spawn effort argument, which is why the rules are written per spawn and degrade here rather than being deleted.

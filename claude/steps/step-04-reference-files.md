...
- **Common Task Patterns**: Table mapping 12 task patterns (System architecture, Security review, Data pipeline, etc.) to required stakeholders beyond mandatory pair. The **Data pipeline / ETL design** row MUST list SRE alongside Data Architect and DevOps Engineer (source: `D24-triage-engine.md` § Common Task Patterns, "Data pipeline design -> Data, Operations, Reliability"). Rationale: a pipeline is a production system whose physical write path can saturate, and the reliability lens ("surprise saturation is operational failure") is what catches concurrent-writer commit contention. Without it, no default pipeline-design stakeholder owns physical capacity.
...
**Must include**:
- **Path-resolution preamble**: state the `.claude/...` paths in the doc are the DEFAULT layout and that `oj-helper resolve-path <key>` is the runtime source of truth. Enumerate both key families - state keys (`session`, `backlog`, `artifacts`, `state-dir`, `config`, `retros`) and category keys (`decisions`, `facts`, `open-questions`, `requirements`, `design`, `plan`) - and name `<root>/.claude/oj-paths.env` as the override point.
- **Filing Rule** (§ *Filing Rule - where a produced document goes*): the canonical decision procedure for where a produced document lands. This section is the single source; the cycle / run-task / spec / workstream-new skills cross-reference it and MUST NOT duplicate it. It must state:
  1. **Classify by type**, not by which skill produced it or what subject it covers: decision, fact, requirement, design, plan, review, analysis.
  2. **Resolve that type's key** via `oj-helper resolve-path <type>`, with `--node <relpath>` naming the node that owns the subject matter.
  3. **On exit 0**, append to or create the resolved file at the owning node - appending to an existing typed file is the normal case, not an exception.
  4. **On exit 3** (a `flat`-layout project with no per-type filing surface), fall back to one document under `oj-helper resolve-path artifacts` (fallback `.claude/artifacts/`). This fallback is what makes the rule non-breaking for projects that never adopted a taxonomy, and it MUST be stated as such.
  - `review` and `analysis` deliberately have no category key.
  - **"A review is not a design"**: point-in-time findings ABOUT existing work are ephemeral and belong in a history area (`resolve-path retros`, or the artifacts root); new content - what the thing is and should be - belongs at the node in `design.md` / `decisions.md` / `requirements.md`. A document containing both MUST be split. State the cost, because it is the reason the rule exists: filing genuine design content as a review buries it in a dated folder nobody cites, so it stops being the answer to "what is the current design?" while still being that answer.
  - **"A fact needs provenance"**: where the target project's `facts/` area requires a provenance marker (check for a `README.md` in the resolved `facts` directory), a skill appending a bare number is producing a defect. Give the common scheme as a table - `MEASURED` (name the command/query/dashboard), `DERIVED` (name the inputs), `ASSERTED-UNVERIFIED` (state the verification path) - and the rule: **prefer `ASSERTED-UNVERIFIED` with a stated verification path over a bare number**, because an unverified fact that says how to verify itself is honest while a bare number is indistinguishable from a measured one and quietly becomes load-bearing. Never upgrade a marker without running the check that justifies it.
...
- **Agent Spawning + Model Selection** — the spawning pattern plus the full model-selection content moved from CONDUCTOR Section 7:
  - **[EXTERNAL]** tier table rendered from `platform-snapshot.yaml` `models` (symbolic ids, tiers, cost ratios - do not hardcode), plus "when in doubt" guidance. Immediately after the table, emit an SSOT annotation naming `${CLAUDE_PLUGIN_ROOT}/platform-defaults.yaml` `platform.models` as the single source of truth for the roster (names, tiers, cost ratios), and stating that this table, the function-first rules, and the per-role defaults mirror that roster for inline guidance (edit the YAML and re-render on a roster change, not the tables). SSOT tiebreak: if the introspected `platform-snapshot.yaml` diverges from `platform-defaults.yaml`, `platform-defaults.yaml` wins as SSOT — refresh defaults before regen.
  - **Minimum-Model Floor** subsection (emit immediately after the "when in doubt" line, before the function-first rules): a configurable floor sets the lowest tier any spawn may run on, defined in `${CLAUDE_PLUGIN_ROOT}/platform-defaults.yaml` `platform.model_policy.min_model_tier` (SSOT; this prose mirrors it and re-renders on a policy change). Render the **current** floor value inline (resolve `platform.model_policy.min_model_tier` from `platform-snapshot.yaml`/`platform-defaults.yaml` at generation time — do not hard-code; default is `routine`). State the mechanism: apply the floor as the LAST step of selection, after function rules and role defaults resolve a tier; raise a below-floor tier up to the floor, leave at-or-above unchanged; the floor is a lower bound only and never lowers an escalation (reviewer slot, Complex-tier lead, domain-decisive-risk specialist still stand). Give the three outcomes resolved to concrete model ids: floor at routine = no bump (identity); floor at implementation = routine spawns bumped up to the implementation-tier id; floor at reasoning = every spawn on the reasoning-tier id. Neutral source: `D32-execution-models.md` §6 "Minimum-Model Floor".
  - **Function-first selection rules** (5 bullets, concrete model ids resolved by looking up `${CLAUDE_PLUGIN_ROOT}/platform-defaults.yaml` `platform.models` for the `{tier-routine}`/`{tier-implementation}`/`{tier-reasoning}` -> id mapping at generation time - do not hard-code the mapping here, it drifts when the roster changes): reviewer-slot → reasoning (always wins); Complex-tier lead → reasoning; Moderate-tier lead → implementation, escalate to reasoning for high-risk / unresolved TENSION; Phase-1 analysts → routine (including bounded/docs-only lenses); domain-trigger specialists → implementation, escalate to reasoning when their domain is the decisive risk.
  - **Per-Role Model Class and Default Tier (adjustable; function rules always win)** table, per D32 §6. Three rows keyed by what the role PRODUCES, not by its seniority: **authoring** class at {tier-implementation} with no turn ceiling (Software Engineer only - its turns produce the deliverable); **advisory, may author** at {tier-routine} with the higher turn ceiling (Distinguished Engineer, Test Engineer, DevOps Engineer, Technical Writer - bounded but NOT tool-restricted, since these four produce a durable artifact often enough that withholding write capability would break real work); **advisory** at {tier-routine} with the lower turn ceiling and artifact-writing tools withheld (Business Analyst, Data Architect, Data Scientist, Engineering Consultant, Enterprise Architect, Executive Leadership Coach, ML Engineer, Product Manager, Security Engineer, Site Reliability Engineer, Solutions Architect). Emit the class names, never concrete model ids. Two qualifiers MUST accompany the table: (a) the function rules above always take precedence when any of them applies (reviewer-slot, Complex-tier lead, Moderate-tier lead, Phase-1 analyst, or domain-trigger specialist), and the per-role default fires only when no function rule matches the spawn; and (b) the agent definitions in `${CLAUDE_PLUGIN_ROOT}/agents/` are the single source of truth for these values - this table mirrors them, and if the two disagree the definitions win and the divergence is a defect. Also emit the negative statement, because it is the whole point of the re-keying: a role no longer earns a deeper default because its subject matter sounds weighty. Reading code and forming a view costs the same whether the view is about security or about naming; depth is bought per spawn by the function rules, where it is load-bearing.
  - **Fan-Out Budget** — the per-invocation spawn/fan-out budget guidance from D32 §6.
  - **Worked-example anchors**: back-pointer to `${CLAUDE_PLUGIN_ROOT}/reference/worked-examples.md` Example 2 - the Phase-1 analysts spawn plainly and run on their frontmatter (advisory class, turn-capped); the Phase-2 lead is the Software Engineer and so is already authoring class, needing no override; the Phase-3 adversarial reviewer is promoted to the authoring class by the reviewer-slot rule. Plus the reviewer-slot-override anchor: a role in the advisory class, spawned as the adversarial reviewer, is promoted for that spawn - the slot takes its class and tier by function, not by role, so do not read Example 2's reviewer as its role's default.
  - **Effort-binding-site note**: render from `platform.effort_binding`. On this target it is `per-agent`, so state plainly how effort resolves at BOTH sites. Per expert: each role declares `effort` in its own definition and that declaration overrides session effort for the duration of the spawn; the spawn CALL still takes no effort argument, so a one-off override means editing the role or spawning a different one, but `model` IS overridable per spawn. Per session: `platform.engagement_effort` sets the manager's own turns, once, at triage. The note MUST carry two prohibitions. First, **do not raise session effort mid-run** - changing effort between requests invalidates the prompt cache, so a mid-engagement raise re-writes the accumulated prefix at cache-write price; if triage re-classifies upward, finish or stop the invocation and start the next one higher. Second, **do not fabricate per-spawn effort control** - naming an effort level in a spawn prompt still documents intent rather than setting a parameter.

    The note MUST also correct the record rather than silently changing it, because the previous value was not a gap but an asserted impossibility that downstream prose built on. State that this target was previously recorded as `session` on the reasoning that profiles reached experts only through the Onboard (`SubagentStart`) injection hook and that `agents/*.md` frontmatter was therefore inert; that the profiles are in fact registered as first-class subagent types and spawned by type, so their `model` / `effort` / `maxTurns` / `tools` / `disallowedTools` / `skills` all take effect (only `permissionMode`, `mcpServers` and `hooks` are ignored for plugin subagents); and that the injection hook remains the fallback for `general-purpose` spawns, which is the only matcher it declares. Other generator targets (see `juntogen/codex/`) expose a per-spawn effort argument, which is why the rules are written per spawn and resolve against the role declaration here rather than being deleted.

  - **Turn-ceiling note**: emit alongside the model-class content. Class sets the price per token and effort sets how many tokens a turn spends, but neither bounds how many turns a spawn takes - and a sub-agent inherits no conversation history, so it re-reads its own accumulating transcript every turn and its cost climbs steeply with turn count. State that advisory roles therefore carry a `maxTurns` ceiling and are denied artifact-writing tools, while the authoring role is left uncapped because its turns produce code.

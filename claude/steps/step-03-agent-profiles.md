...
- **Physical-design lens (MANDATORY)**: the logical-modeling lens alone leaves the physical properties of the storage substrate unowned by any profile in the roster. Emit all four surfaces: (a) a Core Expertise bullet covering partition and file layout, target file size, compaction, and the table format's commit protocol under concurrent writers; (b) a Quality Standards subsection ("Physical design") asserting that every writer to a shared table is named with its commit cadence and isolation mode, that layout and compaction ownership are decided rather than inherited from defaults, and that concurrent-write behavior states what retries, what fails, and what the caller sees; (c) two Section 10 red flags in active language, one hunting concurrent writers whose commit protocol was never examined, one challenging layout defaults at steady state rather than at launch; (d) a Section 12 key question on colliding concurrent commits to the same table.
- **Limitations bullet must not suppress the finding**: the Section 11 bullet on unobservable data distributions, skew, and query plans MUST bound the *magnitude* of a claim only, and MUST say so explicitly (name the physical limit and the measurement that would settle it, rather than omitting the constraint because it cannot be sized). A bare "performance claims are hypotheses" instructs the persona to discount exactly the physical constraints this profile is responsible for raising.
- **SRE handoff trigger**: the Section 6 SRE row fires on concurrent-writer contention or physical layout capping throughput, not only on an unachievable freshness SLA. This pulls the saturation lens in on demand instead of adding SRE to every data-design roster.

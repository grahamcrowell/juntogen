...
- **Model and Effort Selection Section**: the [EXTERNAL] tier table from `platform-snapshot.yaml` (cognitive-demand classes; no cost-ratio column), the effort-binding-site note derived from `platform.effort_binding`, the function-first selection rules (5 bullets), the per-role model-class-and-tier table, the worked-example anchors, and the effort-application note.

#### Spawn Economics (CORE — emitted into CONDUCTOR, NOT moved)

Unlike the mechanics above, three spawn rules DO belong in CONDUCTOR, because they bind outside orchestrated commands where the on-demand reference is never loaded. Emit a short `### Spawn Economics` subsection under Absolute Constraints, immediately after the Delegation Boundary, stating: (1) **named roles carry their own configuration** — each profile declares its `model`, `effort` and turn ceiling, so spawn a named role plainly and pass `model` only to override, never as boilerplate; (2) **ad-hoc spawns are governed too** — any spawn that is not a named `oj:` role MUST pass an explicit `model`, defaulting to the advisory class unless it will write code, because otherwise a policy written about the roster misses a delegation surface comparable in size to the roster itself; (3) **a brief names files, not topics** — a sub-agent starts with a fresh context and no conversation history, so an under-scoped brief yields not a cheap vague answer but an expensive thorough one, the expert compensating by exploring; give it the files, the one question, and permission to stop rather than go looking. Also point ad-hoc search fan-out at the platform's built-in exploration subagent rather than a general-purpose spawn.

**Name model CLASSES here, never a concrete model id.** CONDUCTOR is swept by the s16 assertion and by `vocabulary-audit`; a model id in this file is a build failure, and it would also impose one adopter's procurement policy on every other (Axiom 7).

**Respect the injected-payload byte budget.** CONDUCTOR rides the cached prefix of every turn, so its size is itself a cost control and the `tier-a` payload assertion enforces a ceiling on it. If this subsection would push the rendered payload past that ceiling, displace lower-value prose rather than dropping these rules or letting the budget assertion fail.
...
- **Moderate runs the mandatory pair inline**: the Moderate paragraph MUST state that the mandatory Product + Distinguished pair is applied inline as PERSPECTIVE blocks, as at Simple tier, and that Phase 1 delegates only stakeholders who would read different files than the manager already has, capped at three. It MUST also state that Phases 2 and 3 remain delegated — the adversarial reviewer especially, since it must be able to break work it did not do. That contrast is the point: the delegation boundary is load-bearing where a reviewer needs independence from the work, and decorative where two lenses re-read the manager's own files.
